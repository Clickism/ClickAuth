...
public abstract class MessageType {
    public static final MessageType CONFIRM = new MessageType("&a[✔] ", "&8< &a%s &8>") {
        ...
        public void playSound(Player player) {
            ...
            Bukkit.getScheduler().runTaskLater(ClickAuth.INSTANCE, task -> {
    ...
    protected void send(CommandSender sender, String message, SendType sendType, boolean silent) {
        switch (sendType) {
            case CHAT -> sender.sendMessage(colorize(prefix + message));
            case ACTION_BAR -> {
                if (!(sender instanceof Player player)) break;
                String formatted = String.format(titleFormat, colorize(message));
    ...
    protected void sendTitle(Player player, String title, String subtitle, int fadeIn, int stay, int fadeOut, boolean silent) {
        String titleMessage = title.isEmpty() ? "" : String.format(titleFormat, colorize(title));
        String subtitleMessage = subtitle.isEmpty() ? "" : String.format(subtitleFormat, colorize(subtitle));
